...
function criarGraficoViaAdministracao() {
    const element = document.getElementById('chart-via-administracao');
    if (!element) return;
...
function criarGraficoDrogasDestaque() {
    const element = document.getElementById('chart-drogas-destaque');
    if (!element) return;
    const dados = [
...
function carregarTabelasVias() {
    Object.keys(dadosVias).forEach(via => {
        if (dadosVias[via] && dadosVias[via].length > 0) {
            ...
            if (tbody) {
                // Filtrar dados válidos (excluir linha "Total" e linhas vazias)
                const dados = dadosVias[via].filter(d => 
                    d['Princípio Ativo'] && 
                    d['Princípio Ativo'].trim() !== '' && 
                    d['Princípio Ativo'] !== 'Total' &&
                    !isNaN(parseFloat(d['Soma de VL_APROVADO']))
                ).slice(0, 50); // Limitar a 50 registros para performance
                
                tbody.innerHTML = dados.map(d => `
                    <tr>
                        <td data-sort="${d['Nome Comercial'] || 'N/A'}">${d['Nome Comercial'] || 'N/A'}</td>
                        <td data-sort="${d['Princípio Ativo'] || 'N/A'}">${d['Princípio Ativo'] || 'N/A'}</td>
                        <td data-sort="${parseFloat(d['Soma de VL_APROVADO'] || 0)}">${formatarMoeda(parseFloat(d['Soma de VL_APROVADO'] || 0))}</td>
                        <td data-sort="${parseInt(d['Soma de QT_APROVADA'] || 0)}">${formatarNumero(parseInt(d['Soma de QT_APROVADA'] || 0))}</td>
                        <td data-sort="${parseFloat(d['VL_UNITÁRIO'] || 0)}">${formatarMoeda(parseFloat(d['VL_UNITÁRIO'] || 0))}</td>
                        <td data-sort="${parseFloat(d['Soma de VL_GLOSADO'] || 0)}">${formatarMoeda(parseFloat(d['Soma de VL_GLOSADO'] || 0))}</td>
                        <td data-sort="${parseFloat(d['% GLOSA'] || 0)}">${parseFloat(d['% GLOSA'] || 0).toFixed(2)}%</td>
                    </tr>
                `).join('');
                
                // Inicializar ordenamento para esta tabela
                inicializarOrdenamento(`#tabela-${via.toLowerCase()}`);
            }
        }
    });
}
...
// Função para inicializar ordenamento das tabelas
function inicializarOrdenamento(selector) {
    ...
    headers.forEach(header => {
        header.addEventListener('click', () => {
            ...
            // Remover classes de ordenamento de todos os cabeçalhos
            headers.forEach(h => {
            ...
            // Ordenar tabela
            ordenarTabela(tbody, getColumnIndex(column), newOrder);
        });
    });
}

// Função para obter índice da coluna baseado no data-column
...
function ordenarTabela(tbody, columnIndex, order) {
    const rows = Array.from(tbody.querySelectorAll('tr'));
    
    rows.sort((a, b) => {
        const aValue = a.children[columnIndex].getAttribute('data-sort') || a.children[columnIndex].textContent;
        const bValue = b.children[columnIndex].getAttribute('data-sort') || b.children[columnIndex].textContent;
        ...
        if (!isNaN(aNum) && !isNaN(bNum)) {
            ...
            comparison = aValue.toString().localeCompare(bValue.toString(), 'pt-BR');
        }
